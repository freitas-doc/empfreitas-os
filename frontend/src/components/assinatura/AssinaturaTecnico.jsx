...
export default function AssinaturaTecnico() {
  const { state, dispatch } = useOSForm();
  const [modalAberto, setModalAberto] = useState(false);

  const assinatura  = state.assinaturas.tecnico;
  const nome        = state.responsavel_tecnico_nome || '';
  const cpf         = state.responsavel_tecnico_cpf  || '';
  ...
  const handleSalvar = (base64) => {
    dispatch({ type: 'SALVAR_ASSINATURA', tipo: 'tecnico', base64 });
  ...
  return (
    <div className="card" style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          <h3 className="title-sm" style={{ color: preenchido ? 'var(--color-success)' : 'var(--color-text)' }}>
            {preenchido ? '✔ Assinatura do Tecnico' : 'Assinatura do Tecnico'}
          </h3>
          <p style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>
            Responsavel pela execucao do servico
          </p>
        </div>
      </div>

      {/* Campos de identificação do técnico */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
        <div>
          ...
          <input
            ...
            type="text"
            value={nome}
            onChange={(e) => set('responsavel_tecnico_nome', e.target.value)}
            placeholder="Nome completo do tecnico responsavel"
            style={styleInput}
          />
        </div>
        <div>
          ...
          <input
            id="responsavel-tecnico-cpf"
            type="text"
            inputMode="numeric"
            value={cpf}
            onChange={(e) => set('responsavel_tecnico_cpf', mascaraCPF(e.target.value))}
      ...
      {modalAberto && (
        <PainelAssinatura
          titulo="Assinatura do Tecnico"
          descricao="Atesta a conclusao e responsabilidade pelo servico prestado."
